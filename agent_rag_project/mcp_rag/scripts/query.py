...
"""
# 1. 最简单的查询（使用默认集合）
python scripts/query.py --query "which paper talk about Hierarchical Topic Modeling"

# 2. 指定集合名称
python scripts/query.py --query "which paper talk about Hierarchical Topic Modeling" --collection paper

# 3. 显示详细结果（包括 dense/sparse/fusion/rerank 各阶段）
python scripts/query.py --query "RRF 是什么" --verbose
