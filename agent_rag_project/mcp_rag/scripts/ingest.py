...
def print_summary(results: List[PipelineResult], verbose: bool = False) -> None:
    ...
    # Load 阶段统计
    skipped_before_load = sum(
        1
        for r in results
        if r.stages.get("integrity", {}).get("skipped") is True
    )

    load_success = sum(
        1
        for r in results
        if r.stages.get("loading", {}).get("success") is True
    )

    load_failed = sum(
        1
        for r in results
        if r.stages.get("loading", {}).get("success") is False
    )
    ...
    print(f"  [OK] Pipeline successful: {successful}")
    print(f"  [FAIL] Pipeline failed: {failed}")

    print("\nDOCUMENT LOADING SUMMARY")
    print(f"  [OK] Load success: {load_success}")
    print(f"  [FAIL] Load failed: {load_failed}")
    print(f"  [SKIP] Skipped before load: {skipped_before_load}")
    ...
    if verbose and failed > 0:
        ...
        for r in results:
            if not r.success:
                loading_error = r.stages.get("loading", {}).get("error")
                error = loading_error or r.error
                print(f"  [FAIL] {r.file_path}: {error}")
    ...
    if verbose and successful > 0:
        ...
        for r in results:
            if r.success:
                ...
                loaded = r.stages.get("loading", {}).get("success", False)

                if skipped:
                    status = "[SKIP] skipped before load"
                elif loaded:
                    status = f"[OK] loaded, {r.chunk_count} chunks"
                else:
                    status = f"[OK] {r.chunk_count} chunks"
