...
这是真正干活的文件。它是 BaseVectorStore 的一个具体实现，专门用来操作 ChromaDB 数据库。
"""
...
class ChromaStore(BaseVectorStore):
    ...
    def __init__(self, settings: Settings, **kwargs: Any) -> None:
        ...
        Args:
            settings: Application settings containing vector_store configuration.从配置文件读取参数
            **kwargs: Optional overrides for collection_name or persist_directory.允许运行时覆盖配置
        ...
        # Initialize ChromaDB client with persistent storage创建持久化ChromaDB客户端设置匿名遥测关闭，之后下面的代码才可以在里面创建集合
        try:
            self.client = chromadb.PersistentClient(
                path=str(self.persist_directory), # 此时数据库内部表结构：collections、embeddings、segments(分段）已存在但为空
                settings=ChromaSettings( # 配置客户端的行为选项
        ...
        # Get or create collection获取或创建指定名称的集合，设置余弦相似度计算方式
        try:
            #如果指定名称的集合已存在，直接返回该集合的引用，如果指定名称的集合不存在，自动创建一个新的集合
            self.collection = self.client.get_or_create_collection( #此后collections有了第一条内容：self.collection_name
    ...
    def upsert(
        ...
        **kwargs: Any,
        ...
        Args:
            records: List of records to upsert. Each record must have:待处理的记录列表，每个记录包含id、vector、metadata
                - 'id': Unique identifier (str)
                - 'vector': Embedding vector (List[float])
                - 'metadata': Optional metadata dict
            trace: Optional TraceContext for observability (reserved for Stage F).预留参数，用于后期监控追踪
            **kwargs: Provider-specific parameters (unused for Chroma).提供商特定参数
        ...
        embeddings = []
        metadatas = []
        documents = []  # ChromaDB requires documents field text or id,存text，没text存id(text为分段后的文本，一个段落一个id )
        ...
        **kwargs: Any,
        ...
        if results and results['ids'] and results['ids'][0]:
            ...
            for i, record_id in enumerate(ids):
                ...
                # 余弦距离范围：[0, 2] - 0：完全相同（向量夹角0°）- 1：垂直（向量夹角90°） - 2：完全相反（向量夹角180°）
                # 用户更习惯相似度概念 相似度范围：[0, 1] - 1.0：完全相似- 0.5：中等相似 - 0.0：完全不相似
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self.collection = self.client.get_or_create_collection(
                ...
                metadata={"hnsw:space": "cosine"}# space代表寻找近，用hnsw算法（Hierarchical Navigable Small World（分层可导航小世界））
            )
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        将元数据转换为ChromaDB兼容的格式：
        直接支持：str, int, float, bool → 直接保留
        跳过处理：None → 跳过不存储
        数组转换：list, tuple → 逗号分隔的字符串
        兜底转换：其他类型 → 字符串形式
        """
        sanitized = {}
    ...
    def _build_where_clause(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        将标准过滤条件转换为ChromaDB查询格式：
        简单过滤：{'source': 'doc.pdf'} → {'source': 'doc.pdf'}
        操作符格式：{'score': {'$gt': 0.5}} → 保持原格式
        未来扩展：预留复杂查询操作符支持
        """
