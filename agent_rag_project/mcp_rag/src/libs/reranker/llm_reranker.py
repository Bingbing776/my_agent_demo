...
class LLMReranker(BaseReranker):
    ...
    def __init__(
        self,
        settings: Any,
        prompt_path: Optional[str] = None, # 提示词文件路径
        llm: Optional[BaseLLM] = None,  # llm实例
        **kwargs: Any
    ) -> None:
    ...
    def _build_rerank_prompt(self, query: str, candidates: List[Dict[str, Any]]) -> str:
        ...
        # 整理候选集：遍历所有候选文档（Candidates），提取它们的 id 和 text 格式化
        candidates_text = []
        ...
        # Construct full prompt 拼接全文：把读取到的 prompt_template（题目要求）、query（用户问题）和上面整理好的 Passages（参考材料）拼接在一起 并加一句指令
    ...
    def _parse_llm_response(self, response_text: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            ...
            函数最终必须返回一个 Python 列表（List），列表里的每一项都是一个 字典（Dict），且每个字典必须包含两个键：
                "passage_id"：对应文档的 ID。
                "score"：对应的相关性分数（数字）。
        ...
        # Parse JSON 调用 json.loads()，把纯文本字符串变成 Python 的数据结构（List/Dict）
        try:
    ...
    def _map_results_to_candidates(
        ...
        """
        # Create mapping from id to candidate 先遍历原始 candidates，建立一个 id -> candidate 的字典，方便快速查找。
        id_to_candidate = {}
        for i, candidate in enumerate(candidates):
            candidate_id = candidate.get("id", f"passage_{i}")
            id_to_candidate[candidate_id] = candidate
        
        # Build reranked list 遍历 AI 返回的 parsed_results（包含 id 和 score），根据 id 找到原始文档，给它加一个新字段 rerank_score。
        reranked = []
