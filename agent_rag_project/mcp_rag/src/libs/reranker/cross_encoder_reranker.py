...
class CrossEncoderReranker(BaseReranker):
    ...
    def _get_model_name_from_settings(self, settings: Any) -> str:
        ...
        专门负责从配置文件（settings.yaml）里把模型的名字（比如 BAAI/bge-reranker-base）抠出来
        Args:
    ...
    def _load_cross_encoder_model(self, model_name: str) -> Any:
        """Load the Cross-Encoder model.这是真正去下载或读取 AI 模型文件的函数
        ...
        """
        try:# sentence-transformers 库会自动处理底层的 Tokenization（分词）、Tensor（张量）转换，并调用 PyTorch 计算。
        ...
        try:
            ...
            model = CrossEncoder(model_name)  # 根据model_name找到对应文件
    ...
    def _score_pairs(
        ...
        它把上面做好的“配对列表”喂给 AI 模型（self.model.predict）。模型会计算出每一对的“相关性分数”（比如 0.1, 0.8, 0.5）
        Args:
    ...
    def _attach_scores_and_sort(
        ...
        贴标签：把刚才算出来的分数（0.8）贴回对应的文档上（存入 rerank_score 字段）。
        排序：按分数从高到低排个队（reverse=True）。
        截断：只留下前 K 个最相关的文档（比如只留前 5 个），扔掉后面的垃圾数据。
        Args:
    ...
    def _log_trace(
        ...
        它会记下“这次查询用了多少个候选文档，最后剩了几个”，方便开发人员在后台看系统运行得对不对
        Args:
