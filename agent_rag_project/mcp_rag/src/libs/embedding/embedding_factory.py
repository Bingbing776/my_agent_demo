...
class EmbeddingFactory:
    """Factory for creating Embedding provider instances.注册表存储所有支持的提供者
    ...
    def register_provider(cls, name: str, provider_class: type[BaseEmbedding]) -> None:
        """Register a new Embedding provider implementation.注册一个新的嵌入提供者实现，加入字典，此刻并没有实例化
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseEmbedding:
        """Create an Embedding instance based on configuration.根据配置创建嵌入实例
        ...
        # Instantiate the provider创建实例并返回
    ...
    def list_providers(cls) -> list[str]:
        """List all registered provider names. 列出所有已注册的提供者名称
...
# Auto-register providers on module import 自动注册内置的嵌入提供者
...
# Register providers when module is imported 模块被导入时自动执行
