...
class OpenAIEmbedding(BaseEmbedding):
    """OpenAI Embedding provider implementation. 鏀寔鏍囧噯OpenAI API鍜孉zure鍏煎妯″紡
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        api_params = {
            "input": texts,
            "model": self.model,
            "encoding_format": "float",  # DashScope 兼容模式要求
        }
        ...
        if dimensions is not None and self.model.startswith("text-embedding-v4"):
            api_params["dimensions"] = dimensions
        
        if len(texts) > 10:
            raise ValueError(
                f"text-embedding-v4 allows at most 10 inputs per request, got {len(texts)}"
            )
        ...
        logger.debug("[embed] model=%s, dims=%s, count=%d", self.model, api_params.get('dimensions'), len(texts))
        for i, t in enumerate(texts):
            logger.debug("[embed] text[%d] chars=%d", i, len(t))
        ...
        try:
            ...
            raise OpenAIEmbeddingError(
                f"OpenAI Embeddings API call failed: {e}; response={detail}"
            ) from e
        """except Exception as e:
            raise OpenAIEmbeddingError(
