"""Azure OpenAI Embedding implementation.实现Azure OpenAI的嵌入服务，处理Azure特定配置（端点、API版本、部署名）
...
class AzureEmbedding(BaseEmbedding):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        # Azure uses 'deployment_name' instead of 'model' 部署名
        ...
        # API key: explicit parameter > settings.yaml > env var (fallback for backward compatibility)获取API秘钥
        self.api_key = (
            api_key or 
        ...
        # Azure endpoint: explicit parameter > settings.yaml > env var (fallback)获取端点，即为云端地址
        self.azure_endpoint = (
            azure_endpoint or
        ...
        # API version: explicit > settings > default API版本
        self.api_version = (
            api_version or
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        # Add dimensions if specified (only for text-embedding-3-* models)避免不支持维度参数的模型传入了维度
    ...
    def get_dimension(self) -> Optional[int]:
        ...
        """
        # If dimensions explicitly configured, return it这里设计有问题
        if self.dimensions is not None:
            return self.dimensions
        
        # Common Azure deployment defaults 根据部署名推断默认维度
