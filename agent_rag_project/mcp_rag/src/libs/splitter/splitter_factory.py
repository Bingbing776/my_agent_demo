...
def _register_builtin_providers() -> None:
    """Register built-in splitter providers. 自动注册内置的分割器实现
    
    This function is called automatically when the module is imported.在模块导入时自动调用
    It registers all available splitter implementations with the factory. 它会在工厂中注册所有可用的拆分器实现。
    """
...
class SplitterFactory:
    """Factory for creating Splitter provider instances. 向工厂注册新的分割器实现
    ...
    def register_provider(cls, name: str, provider_class: type[BaseSplitter]) -> None:
        """Register a new Splitter provider implementation.向工厂注册新的分割器实现
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseSplitter:
        """Create a Splitter instance based on configuration.根据配置创建分割器实例
    ...
    def list_providers(cls) -> list[str]:
        """List all registered provider names.返回所有已注册的提供者名称列表
