...
class OpenAIVisionLLM(BaseVisionLLM):
    ...
    初始化OpenAI视觉客户端 - 设置API密钥、模型、URL、图片大小限制等配置。
    ...
    def chat_with_image(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            content = response_data["choices"][0]["message"]["content"]  # 这是 AI 真正回复的文字内容
            usage = response_data.get("usage")  # 这是账单信息 记录了这次对话消耗了多少“输入令牌”和“输出令牌”，用来计算成本和监控配额
