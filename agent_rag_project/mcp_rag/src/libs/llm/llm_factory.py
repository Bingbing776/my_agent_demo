...
def _register_vision_providers() -> None:
    ...
    自动注册视觉模型 - 在模块加载时自动注册Azure和OpenAI视觉模型。
...
class LLMFactory:
    """Factory for creating LLM provider instances.注册表存储
