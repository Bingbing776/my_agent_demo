...
def _snapshot_results(
    ...
    把 RetrievalResult 对象(稠密or稀疏)列表转成一个可序列化的简化字典列表，方便写入 trace/debug 日志。
    Args:
...
class HybridSearchConfig:
    ...
    """
    dense_top_k: int = 20  # dense 检索先取多少条
    sparse_top_k: int = 20
    fusion_top_k: int = 10  # 融合后最终保留多少条
    enable_dense: bool = True  # 是否启用 dense
    enable_sparse: bool = True
    parallel_retrieval: bool = True  # 是否并行跑两路
    metadata_filter_post: bool = True  # 融合后是否再做一次 metadata filter
...
class HybridSearchResult:
    ...
    """
    results: List[RetrievalResult] = field(default_factory=list)  # 检索结果的最终排名列表
    dense_results: Optional[List[RetrievalResult]] = None  # 稠密检索结果（用于调试）
    sparse_results: Optional[List[RetrievalResult]] = None  # 稀疏检索结果（用于调试）
    dense_error: Optional[str] = None  # 稠密检索失败时的错误消息
    sparse_error: Optional[str] = None  # 稀疏检索失败时的错误消息
    used_fallback: bool = False  # 是否使用了回退模式(是否只使用其中一种检索结果)
    processed_query: Optional[ProcessedQuery] = None  # 处理后的查询（用于调试）
...
class HybridSearch:
    ...
    def _build_rerank_query(self, processed_query: ProcessedQuery) -> str:
        """Build enhanced query for reranking.

        Reranker sees:
        original query + MQE queries + HyDE passage
        """
        parts: List[str] = [processed_query.original_query]

        if processed_query.expanded_terms:
            parts.append("Expanded retrieval queries:")
            parts.extend(processed_query.expanded_terms)

        hyde_passage = getattr(processed_query, "hyde_passage", None)
        if hyde_passage:
            parts.append("Hypothetical evidence passage:")
            parts.append(hyde_passage)
    ...
    def search(
        ...
        Returns:
            If return_details=False: List of RetrievalResult sorted by relevance. List[RetrievalResult]
            If return_details=True: HybridSearchResult with full details. HybridSearchResult
        ...
        """
        # Validate query 如果 query 为空或全空格，直接报：
        ...
        # Step 1: Process query 这里会把原始 query 变成结构化形式
        _t0 = time.monotonic()
        ...
        # Merge explicit filters with query-extracted filters把两类 filter 合并：query 里解析出来的 filter，调用方显式传入的 filter，并且显式传入的优先级更高
        merged_filters = self._merge_filters(processed_query.filters, filters)
        
        # Step 2: Run retrievals 根据配置决定并行跑还是串行跑
        ...
        # Step 3: Handle fallback scenarios 降级处理逻辑
        used_fallback = False
        ...
        # Step 5: Apply post-fusion metadata filters (if any) 后置 metadata 过滤
        ...
        # Step 6: Limit to top_k 截断 top_k
        final_results = fused_results[:effective_top_k]

        for r in final_results:
            r.metadata["mqe_queries"] = processed_query.expanded_terms or []
            r.metadata["hyde_passage"] = getattr(processed_query, "hyde_passage", None)
    ...
    def _process_query(self, query: str) -> ProcessedQuery:
        ...
        if self.query_processor is None:
            # Fallback: create basic ProcessedQuery 如果没有配置 query_processor，它会降级成非常简单的逻辑
    ...
    def _run_retrievals(
        ...
        """Run Dense and Sparse retrievals with split MQE / HyDE logic.

        Dense:
            original query + MQE queries + HyDE passage

        Sparse/BM25:
            original query + MQE queries only
        """
        dense_results_all: List[RetrievalResult] = []
        sparse_results_all: List[RetrievalResult] = []
        ...
        if not run_dense and not run_sparse:
            if self.dense_retriever is None and self.sparse_retriever is None:
                return None, None, "No retriever configured", "No retriever configured"
            return None, None, None, None

        # MQE 给 dense + sparse 都用
        mqe_queries = processed_query.expanded_terms or []

        # HyDE 只给 dense 用
        hyde_passage = getattr(processed_query, "hyde_passage", None)

        dense_candidate_queries: List[str] = [processed_query.original_query]
        dense_candidate_queries.extend(mqe_queries)
        if hyde_passage:
            dense_candidate_queries.append(hyde_passage)

        sparse_candidate_queries: List[str] = [processed_query.original_query]
        sparse_candidate_queries.extend(mqe_queries)

        dense_queries = self._dedupe_queries(dense_candidate_queries)
        sparse_queries = self._dedupe_queries(sparse_candidate_queries)

        if trace is not None:
            trace.record_stage(
                "query_expansion",
                {
                    "method": "mqe_hyde_split",
        ...
        # Dense retrieval: original + MQE + HyDE
        if run_dense:
            for q in dense_queries:
                ...
                if dense_results:
                    dense_results_all.extend(dense_results)
                if dense_error:
                    dense_errors.append(dense_error)

        # Sparse retrieval: original + MQE only
        if run_sparse:
            for idx, q in enumerate(sparse_queries):
                if idx == 0:
                    sub_processed = processed_query
                elif self.query_processor is not None:
                    tokens = self.query_processor._tokenize(q)
                    keywords = self.query_processor._filter_keywords(tokens)
                    sub_processed = ProcessedQuery(
                        original_query=q,
                        keywords=keywords,
                        filters={},
                        expanded_terms=[],
                ...
                if sub_processed.keywords:
                    sparse_results, sparse_error = self._run_sparse_retrieval(
                        sub_processed.keywords,
                        filters,
                        trace,
                    )
                    if sparse_results:
                        sparse_results_all.extend(sparse_results)
                    if sparse_error:
                        sparse_errors.append(sparse_error)

        dense_results_final = self._deduplicate_results(dense_results_all)
        sparse_results_final = self._deduplicate_results(sparse_results_all)

        dense_error_final = (
            "; ".join(dense_errors)
            if dense_errors and not dense_results_final
            else None
        )
        sparse_error_final = (
            "; ".join(sparse_errors)
            if sparse_errors and not sparse_results_final
            else None
        )

        return (
            dense_results_final,
            sparse_results_final,
            dense_error_final,
            sparse_error_final,
        )
    ...
    def _deduplicate_results(
        ...
        """Deduplicate retrieval results by chunk_id, preserving best score."""
        if not results:
            return []
    ...
    def _matches_filters(
        ...
        判断某条结果的 metadata 是否满足所有 filter 条件。
        Args:
...
def create_hybrid_search(
    ...
    快速创建HybridSearch，如果没穿fusion会默认RRFFusion(k=60)
