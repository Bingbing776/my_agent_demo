...
class RerankError(RuntimeError):
    """Raised when reranking fails.一个自定义异常类"""
...
class RerankResult:
    ...
    rerank 的结果对象
    Attributes:
        results: Reranked list of RetrievalResults 重排后的结果
        used_fallback: Whether fallback was used due to backend failure 是否用了 fallback
        fallback_reason: Reason for fallback (if applicable) fallback 原因
        reranker_type: Type of reranker used ('llm', 'cross_encoder', 'none') 实际用的 reranker 类型
        original_order: Original results before reranking (for debugging) 重排前原始顺序
    """
...
class CoreReranker:
    ...
    def _extract_config(self, settings: Settings) -> RerankConfig:
        ...
        except AttributeError:# 如果 settings 没有 rerank 配置，它会：打 warning返回 RerankConfig(enabled=False)也就是默认禁用 rerank。
    ...
    def _get_reranker_type(self) -> str:
        ...
        记录当前 backend 是：llm cross_encoder none 或其他类名派生值
        Returns:
    ...
    def _results_to_candidates(self, results: List[RetrievalResult]) -> List[Dict[str, Any]]:
        ...
        把 RetrievalResult 列表转成 reranker backend 需要的候选格式
        因为底层 reranker 通常更喜欢吃“字典候选列表”，而不是你 core 层的 dataclass 对象
        Args:
    ...
    def _candidates_to_results(
        ...
        把 reranker 返回的候选结果，再转回 RetrievalResult。
        Args:
    ...
    def rerank(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            _t0 = time.monotonic()
            enhanced_query = self._build_rerank_query(query, results)
    ...
    def _build_rerank_query(self, query: str, results: List[RetrievalResult]) -> str:
        """Enhance query for reranking using MQE / HyDE info stored in metadata.

        Only the first MQE query is used.
        """
        # 默认原 query
        parts = [query]
        ...
        for r in results:
            meta = r.metadata or {}
            if not mqe_queries:
                mqe_queries = meta.get("mqe_queries", [])
            if not hyde_passage:
                hyde_passage = meta.get("hyde_passage")
            if mqe_queries and hyde_passage:
                break

        # 只保留第一条 MQE
        if mqe_queries:
            parts.append("Expanded query:")
            parts.append(mqe_queries[0].strip())

        # HyDE 放最后
        if hyde_passage:
            parts.append("Hypothetical evidence passage:")
            parts.append(hyde_passage.strip())
