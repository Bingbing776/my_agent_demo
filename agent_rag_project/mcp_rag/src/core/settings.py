...
def deep_merge_settings(base: Dict[str, Any], overlay: Dict[str, Any]) -> Dict[str, Any]:
    """Deep-merge overlay dict onto base (for settings.llm_live.yaml overlays)."""
    out = dict(base)
    for key, val in overlay.items():
        if key in out and isinstance(out[key], dict) and isinstance(val, dict):
            out[key] = deep_merge_settings(out[key], val)
        else:
            out[key] = val
    return out
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    Args:
        path: Path to settings YAML.  Defaults to
            ...
            If ``MODULAR_RAG_SETTINGS_PATH`` is set, loads base settings.yaml
            then deep-merges the overlay file (repo-relative path).
    """
    if path is not None:
        ...
        env_overlay = os.environ.get("MODULAR_RAG_SETTINGS_PATH", "").strip()
        base_path = DEFAULT_SETTINGS_PATH
        if not base_path.exists():
            raise SettingsError(f"Settings file not found: {base_path}")
        with base_path.open("r", encoding="utf-8") as handle:
            data = yaml.safe_load(handle) or {}
        if env_overlay:
            overlay_path = Path(env_overlay)
            ...
            if overlay_path.exists():
                ...
                data = deep_merge_settings(data, overlay)
            else:
                raise SettingsError(f"Settings overlay not found: {overlay_path}")
