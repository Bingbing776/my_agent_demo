...
class DocumentChunker:
    ...
    def __init__(self, settings: Settings):
        ...
        print("splitter =", type(self._splitter))
        print("include_references =", getattr(self._splitter, "include_references", None))
        print("include_appendix =", getattr(self._splitter, "include_appendix", None))
    
    def split_document(self, document: Document) -> List[Chunk]:
        """Split a Document into Chunks with full business enrichment.
        这是外部调用的唯一入口。它负责指挥整个流程
        ...
        # Step 1: Use underlying splitter to get text fragments 调用底层切刀把文本切成碎片。
        ...
        # 遍历每一个碎片，调用 _generate_chunk_id 和 _inherit_metadata 给它“穿衣服”（加ID和元数据）。把它们打包成 Chunk 对象列表。
        chunks: List[Chunk] = []
    ...
    def _generate_chunk_id(self, doc_id: str, index: int, text: str) -> str:
        ...
        给每一个切出来的文本块生成一个确定性的、唯一的 ID。
        为什么需要这个？ 因为如果同一个文档被处理两次，我们希望生成的 Chunk ID 是一样的（Deterministic），这样数据库才知道是同一个东西，避免重复存储。
    ...
    def _inherit_metadata(self, document: Document, chunk_index: int, chunk_text: str = "") -> dict:
        ...
        把父文档的“户口本”（metadata）拿过来，撕掉不需要的部分，加上这个小块特有的信息（比如这是第几块、引用了什么图片），然后封装好。
