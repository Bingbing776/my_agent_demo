...
class MetadataEnricher(BaseTransform):
    ...
    Configuration (via settings.yaml):
        - ingestion.metadata_enricher.use_llm: bool - Enable LLM enhancement 启用LLM增强
        - ingestion.metadata_enricher.prompt_path: str - Custom prompt file path 自定义提示文件路径
    ...
    def __init__(
        ...
        """Initialize MetadataEnricher.初始化
    ...
    def _extract_title(self, text: str) -> str:
        ...
        # Check for markdown heading 优先找 Markdown 的 # 标题
        heading_match = re.match(r'^#{1,6}\s+(.+)$', text, re.MULTILINE)
        if heading_match:
            return heading_match.group(1).strip()
        
        # Use first line if it's short and looks like a title 找不到就用第一行
        first_line = text.split('\n')[0].strip() #split('\n')代表按\n切开变成一个列表
        if first_line and len(first_line) <= 100 and not first_line.endswith(('.', ',', ';')):
            return first_line
        
        # Use first sentence (without trailing punctuation) 还不行就截取前100个字
    ...
    def _extract_summary(self, text: str, max_sentences: int = 3) -> str:
        """Extract summary from text using first N sentences.生成摘要
    ...
    def _extract_tags(self, text: str, max_tags: int = 10) -> List[str]:
        ...
        # Extract capitalized words (potential proper nouns) 利用正则表达式找“大写字母开头的词”（可能是专有名词）
        capitalized = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', text)
        tags.update(capitalized[:5])
        
        # Extract code identifiers (camelCase, snake_case) “代码里的变量名”（snake_case）
        identifiers = re.findall(r'\b[a-z]+(?:[A-Z][a-z]*)+\b|\b[a-z]+_[a-z_]+\b', text)
        tags.update(identifiers[:5])
        
        # Extract markdown bold/italic terms (potential keywords) 提取Markdown中的粗体/斜体术语（潜在关键词）
