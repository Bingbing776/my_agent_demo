...
class DenseEncoder:
    ...
    def encode(
        ...
        # Extract text from chunks 把 Chunk 对象剥开，只留下 .text
        texts = [chunk.text for chunk in chunks]
        
        # Validate that all texts are non-empty 如果文本是空的或者只有空格，直接报错。
        ...
        # Final validation 检查输入分块数与返回向量数是否一致
        ...
        # Validate vector dimensions are consistent 检查所有向量长度是否一致
        if all_vectors:
