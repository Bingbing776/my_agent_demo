...
class SparseEncoder:
    ...
    def __init__(
        ...
        Args:
            min_term_length: Minimum character length for a term (default: 2) 最小词长
            lowercase: Whether to convert terms to lowercase (default: True) 转小写
    ...
    def encode(
        ...
        它接收一堆 Chunk 对象，给每个 Chunk 算出一张“统计报表”
        ...
        for i, chunk in enumerate(chunks):
            ...
            # 调用 _tokenize 把句子切碎成词
            terms = self._tokenize(chunk.text)
            # 用 Python 的 Counter 工具数数。比如 "hello world hello" 会变成 {'hello': 2, 'world': 1}
            term_frequencies = Counter(terms)
            
            # Build statistics dict
            stat_dict = {
                "chunk_id": chunk.id,
                "term_frequencies": dict(term_frequencies),  # Convert Counter to dict词频
                "doc_length": len(terms), #文档长度，一共有多少个词
                "unique_terms": len(term_frequencies),  # 词汇量，一共有多少个不重复的词
            }
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        # Use jieba to segment the text (handles both Chinese and English)分词库
        ...
        for token in raw_tokens:
            ...
            if not token:
                continue
            # Skip pure punctuation / whitespace 跳过纯标点和空格
            if re.fullmatch(r'[\s\W]+', token, re.UNICODE):
                continue
            tokens.append(token)
        
        # Apply lowercase if configured 小写
        if self.lowercase:
            tokens = [t.lower() for t in tokens]
        
        # Filter by minimum length 长度小于最小长度的扔掉
    ...
    def get_corpus_stats(
        ...
        Returns:
            ...
            {
                "num_docs": int, 总共有多少个Chunk
                "avg_doc_length": float,  平均chunk长度
        ...
        num_docs = len(encoded_chunks) #  总共有多少个Chunk
        total_length = sum(chunk["doc_length"] for chunk in encoded_chunks)
        avg_doc_length = total_length / num_docs if num_docs > 0 else 0.0  # 平均chunk长度(标尺，看mouchunk是长是短) 把所有文本的词数加起来，除以篇数
        
        # Calculate document frequency (DF) for each term文档频率（这个词在多少篇不同的chunk里出现过）
