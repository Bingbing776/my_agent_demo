...
class BM25Indexer:
    ...
    def __init__(
        self,
        index_dir: str = "data/db/bm25",  # 设置索引文件的保存路径
        k1: float = 1.5, # BM25 算法的两个关键参数 k1（词频饱和度）词频重要性
        b: float = 0.75,  # 文档长度归一化 长度惩罚程度
    ):
    ...
    def build(
        ...
        # Step 1: Calculate corpus-level statistics 统计：计算语料库的总文档数、平均文档长度。
        ...
        # Calculate document frequency (DF) for each term 统计每个词出现在多少篇文档中（文档频率）
        doc_freq: Dict[str, int] = {}
        for stat in term_stats:
            for term in stat["term_frequencies"].keys():
                doc_freq[term] = doc_freq.get(term, 0) + 1
        
        # Step 2: Build inverted index with IDF  为每个词建立一个“词 -> [包含该词的文档列表]”的映射，并计算 IDF
    ...
    def query(
        ...
        for term in query_terms:
            ...
            for posting in term_data["postings"]:  # 遍历每一个搜索词
                chunk_id = posting["chunk_id"]  # 拿到文档的具体信息
                tf = posting["tf"]
                doc_length = posting["doc_length"]
                
                # BM25 score contribution from this term  计算这个词在这个文档里的单项得分
        ...
        # Sort by score descending and return top_k 排序返回topk
    ...
    def add_documents(
        ...
        在不破坏旧索引的情况下，添加新文档或更新旧文档 
        如果提供了 doc_id，它会先尝试从旧索引中删除该 ID 对应的所有旧切片（防止重复），然后将旧数据和新数据合并，最后重新构建索引。
        ...
        # Remove stale postings for this document (re-ingest case)先删除
        if doc_id and self._index:
            self.remove_document(doc_id, collection)

        # Reconstruct existing term_stats from current index postings 倒排还原（因为build只能接收倒排之前的格式）
    ...
    def remove_document(
        ...
        遍历每一个词，把属于该文档的切片从词的列表里踢出去。
        因为文档是“散”在各个词下面的。要删掉一个文档，必须把散落在“苹果”、“香蕉”、“电脑”等所有词下面的该文档记录都找出来删掉。
        删除后，它会重新计算受影响的统计信息（如文档总数 N 和 IDF 值），并保存索引。
        ...
        for term, term_data in self._index.items():
            original_len = len(term_data["postings"])  # 记录删除前的长度
            term_data["postings"] = [  # 使用列表推导式，只保留那些 chunk_id 不以 doc_id 开头的条目
                p for p in term_data["postings"]
                if not p["chunk_id"].startswith(doc_id)  # 检查 chunk_id 这个字符串，是不是以 doc_id 开头的
            ]
            if len(term_data["postings"]) < original_len:  # 检查是否真的删掉了东西
                removed_any = True
            # Mark empty terms for cleanup 如果列表空了，先记下来（不能在遍历字典时直接删除键）
            if not term_data["postings"]:
                terms_to_delete.append(term)
            else:  # 如果列表没空，更新这个词的文档频率（DF）
                   # 因为少了一个文档包含它，DF 必须减 1
                term_data["df"] = len(term_data["postings"])

        # Remove empty terms 执行删除：把那些列表变空的词彻底从索引字典中移除
        for term in terms_to_delete:
            del self._index[term]

        if removed_any:
            # Recalculate global metadata 重新扫描剩下的所有数据，收集最新信息
    ...
    def _calculate_idf(self, num_docs: int, df: int) -> float:
        """Calculate IDF using BM25 formula.计算idf
    ...
    def _calculate_bm25_score(
        ...
        """Calculate BM25 score for a single term in a document.计算某个词在某个文档中的具体得分
    ...
    def _save(self, collection: str) -> None:
        """Save index to disk.
        保存索引，安全的原子性（这个替换动作要么完全成功，要么完全不发生，不会停在中间状态）写入
        因为如果你直接写正式文件，中途一旦出问题，比如：程序崩了机器断电了磁盘异常写到一半报错那正式文件可能会变成：
        半截 JSON空文件损坏文件下次加载索引时就读不了了。
        Args:
            collection: Collection name
        """
        ...
        try:
            ...
            # Atomic rename再把这个临时文件直接替换成正式文件
