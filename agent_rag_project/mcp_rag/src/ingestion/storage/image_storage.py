...
class ImageStorage:
    ...
    def __init__(
        ...
        self.images_root = Path(images_root)  # 图片存储根目录
        self._conn = None
        self._ensure_database()
    
    def close(self) -> None:
        """Close database connection if open.关闭数据库连接"""
        if self._conn:
            self._conn.close()
            self._conn = None
    ...
    def __del__(self):
        ...
        析构函数。对象被销毁时，自动调用 close() 做清理"""
        self.close()
    
    def _ensure_database(self) -> None:
        """Create database file and schema if they don't exist.
        确保数据库文件、父目录、图片根目录、数据表和索引都存在。初始化数据库结构的核心函数"""
    ...
    def save_image(
        ...
        把图片保存到文件系统，并把这张图片登记到 SQLite 数据库里。
        这是“保存文件 + 建索引”的一站式方法。
    ...
    def list_images(
        ...
        列出图片索引信息，可以按 collection 或 doc_hash 过滤。
        Args:
