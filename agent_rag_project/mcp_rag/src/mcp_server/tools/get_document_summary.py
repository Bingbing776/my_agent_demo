...
class GetDocumentSummaryTool:
    ...
    def _get_collection(self, collection_name: Optional[str] = None) -> Any:
        ...
        从 ChromaDB client 里拿到某个 collection 对象。
        Args:
    ...
    def _find_document_chunks(
        ...
        # Strategy 1: Search by source_ref metadata 按 source_ref 查（父文档）
    ...
    def get_document_summary(
        ...
        # Sort chunks by chunk_index if available 按 chunk_index 排序
        chunks.sort(key=lambda c: c.get('metadata', {}).get('chunk_index', 0))
        
        # Extract document-level info from first chunk's metadata 拿第一块的 metadata（因为通常标题、来源路径等文档级信息，第一块里往往已经有。）
    ...
    def _extract_summary(self, chunks: List[Dict[str, Any]]) -> str:
        ...
        2. First N characters from first chunk text 文档前面正文的截断预览
    ...
    def _filter_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        把 metadata 里一些内部字段过滤掉，只保留用户更关心的内容。
        Args:
    ...
    async def execute(
        ...
        try:
            ...
            summary = await asyncio.to_thread(  # 把阻塞的 ChromaDB 查询放到线程里
                self.get_document_summary, doc_id, collection,
            )
            response_text = self.format_response(summary) # 格式化结果
            # 包成 MCP 返回值
