...
class CollectionInfo:
    ...
    """
    name: str # 名字
    count: Optional[int] = None  # 这个collection里有多少文档/块
    metadata: Optional[Dict[str, Any]] = None  # 附加信息
...
class ListCollectionsConfig:
    ...
    """
    persist_directory: str = "./data/db/chroma"  # ChromaDB存数据的目录
    include_stats_default: bool = True  # 默认是否带统计信息
...
class ListCollectionsTool:
    ...
    def __init__(
        ...
        """
        self._settings = settings  # 配置文件
        self._config = config  #工具自己的配置（数据库地址）
    ...
    def _get_chroma_client(self) -> Any:
        ...
        创建一个 ChromaDB 客户端，并让它指向你本地已经保存数据的那个目录。
        Returns:
    ...
    def list_collections(
        ...
        去 ChromaDB 看现在有哪些 collection，然后整理成 CollectionInfo 列表返回
        Args:
        ...
        try:
            # Get all collections from ChromaDB 向 ChromaDB 请求所有 collections
...
def register_tool(protocol_handler: ProtocolHandler) -> None:
    ...
    把 list_collections 注册到 ProtocolHandler 里
    ...
    """
    tool = ListCollectionsTool() # 创建工具对象
    # 定义一个 MCP handler 真正挂给协议层的是这个 handler
    async def handler(
        include_stats: bool = True,
    ) -> types.CallToolResult:
