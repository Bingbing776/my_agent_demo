...
“知识库查询工具”的总控文件：接收用户的查询词；去知识库里搜；必要时再重排结果；把结果包装成 MCP 能返回的格式；返回给外部调用方
...
class QueryKnowledgeHubTool:
    ...
    def __init__(
        ...
        # 表示当前是在为哪个 collection 准备检索组件
        self._current_collection: Optional[str] = None
    
    @property
    def settings(self) -> Settings:
        """Get settings, loading if necessary.懒加载，需要的时候再加载"""
        if self._settings is None:
            # 把配置文件 settings.yaml 读进来，检查有没有问题，然后变成程序里能方便使用的 Settings 对象
    ...
    def _ensure_initialized(self, collection: str) -> None:
        ...
        确保查询相关的组件已经为某个 collection 准备好了
    ...
    async def execute(
        ...
        try:
            ...
            # Build response 构建最终响应，调用response_builder.py中的响应构建器
            ...
            # Store final results in trace for dashboard display结果写trace里
    ...
    def _build_error_response(
        ...
        这个函数专门用来构造“查询失败”的响应。它会生成一段中文 Markdown 包装成 MCPToolResponse 返回 给用户一个可读的错误结果
        Args:
            query: Original query.
...
# Module-level tool instance (lazy-initialized) 维护一个实例
...
async def query_knowledge_hub_handler(
    ...
    给 MCP 协议层注册用的 handler。当 MCP 客户端调用工具 query_knowledge_hub 时，最终会走到这个函数
    ...
    try:
        response = await tool.execute(  # 调工具执行
            query=query,
            top_k=top_k,
            collection=collection,
        )
        
        # 把响应转成 MCP content blocks，tool.execute() 返回的是 MCPToolResponse handler 还要把它变成 MCP 协议真正要的
...
def register_tool(protocol_handler) -> None:
    ...
    把这个工具注册到 ProtocolHandler 登记到 self.tools 里
